...
public class Peerlocal  implements Parcelable {
    public static final Creator<Peerlocal> CREATOR = new PeerProxyCreator();
   ...
   // @Nullable private InterfaceDnsListener interfaceDnsListener;
    @Nullable private Configlocal owner;
    ...
    private Peerlocal(final Parcel in) {
    ...
    public Peerlocal(final Peer other) {
    ...
    public void bind(final Configlocal owner) {
//        final Interfacelocal interfaze = owner.getInterface();
//        final List<Peerlocal> peers = owner.getPeers();
...
//            return;
//        final Interfacelocal interfaze = owner.getInterface();
//        final ObservableList<Peerlocal> peers = owner.getPeers();
//        if (interfaceDnsListener != null)
...
//    private static final class InterfaceDnsListener extends OnPropertyChangedCallback {
//        private final WeakReference<Peerlocal> weakPeerProxy;
//
//        private InterfaceDnsListener(final Peerlocal peerProxy) {
//            weakPeerProxy = new WeakReference<>(peerProxy);
//        }
//
//        @Override
//        public void onPropertyChanged(final Observable sender, final int propertyId) {
//            @Nullable final Peerlocal peerProxy = weakPeerProxy.get();
//            if (peerProxy == null) {
//                sender.removeOnPropertyChangedCallback(this);
//                return;
//            }
//            // This shouldn't be possible, but try to avoid a ClassCastException anyway.
//            if (!(sender instanceof Interfacelocal))
//                return;
//            if (!(propertyId == BR._all || propertyId == BR.dnsServers))
//                return;
//            peerProxy.setInterfaceDns(((Interfacelocal) sender).getDnsServers());
//        }
//    }

    private static final class PeerListListener
            extends ObservableList.OnListChangedCallback<ObservableList<Peerlocal>> {
        private final WeakReference<Peerlocal> weakPeerProxy;

        private PeerListListener(final Peerlocal peerProxy) {
            weakPeerProxy = new WeakReference<>(peerProxy);
        }

        @Override
        public void onChanged(final ObservableList<Peerlocal> sender) {
            @Nullable final Peerlocal peerProxy = weakPeerProxy.get();
            if (peerProxy == null) {
        ...
        @Override
        public void onItemRangeChanged(final ObservableList<Peerlocal> sender,
                                       final int positionStart, final int itemCount) {
            // Do nothing.
        }

        @Override
        public void onItemRangeInserted(final ObservableList<Peerlocal> sender,
                                        final int positionStart, final int itemCount) {
            onChanged(sender);
        }

        @Override
        public void onItemRangeMoved(final ObservableList<Peerlocal> sender,
        ...
        @Override
        public void onItemRangeRemoved(final ObservableList<Peerlocal> sender,
                                       final int positionStart, final int itemCount) {
            onChanged(sender);
        }
    }

    private static class PeerProxyCreator implements Creator<Peerlocal> {
        @Override
        public Peerlocal createFromParcel(final Parcel in) {
            return new Peerlocal(in);
        }

        @Override
        public Peerlocal[] newArray(final int size) {
            return new Peerlocal[size];
        }
    }
}
